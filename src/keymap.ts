 ...
 *   3. agent mid-turn (running)     → cancel the current task
 ...
 * while requiring a deliberate double-press before an idle exit.
 *
...
export type KeyAction =
  ...
  | { kind: 'interrupt-cancel'; consumes: true }     // Esc while running
...
function resolveEscape(state: KeyPressState, now: number): KeyAction {
  ...
  if (state.running) return { kind: 'interrupt-cancel', consumes: true }
  if (state.editorHasText) return { kind: 'noop', consumes: false }
  // Empty editor, idle: first press arms, second within the window fires.
