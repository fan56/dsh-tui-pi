...
| S+ | **widget 位置重做 + header 识别子代理 + 面板边框**（用户实测后反馈三点：Agents 没显示 / Todos 在顶部而非聊天框上方 / 要有边框）：① **定位**——widgets 从 root VStack 顶部移进 dock VStack、编辑器（聊天输入框）正上方（tui.ts：status, widgets, editor, lastRequest, footer；rebuildEditor 同步重加）；② **子代理识别**——根因：该部署的 subagent 走后台 spawn，父日志**没有 tool-workflow/agent-start**（实测 e00b974c 会话事件计数），原追踪器永远看不到子代理。改为**按子会话 header 识别**：session/event 火线回调带完整 session，凡 `header.origin==='subagent'` 且 `parentSession` ∈ trackedSessions 即注册（delegation 嵌套递归）；label/provider 由子会话 subagent/descriptor 补充；`turn/end` 视为结算（best-effort outcome='completed'，turn/start 重新置为运行）；AgentView 改按 childId 键控（去掉 seq，排序按 startedAt），runSeqToChild 保留 tool-workflow agent-end 配对；③ **边框**——两个面板各自成框（复用 panelBoxWidth/panelBodyText/clipPanelLine，本地 top/header 行；borderDefault 边框、无背景）；行宽预算按 boxed inner cap = panelBoxWidth-4 算（todo content cap-5、agent name=provider+label 共享 nameBudget、activity cap-8），病理内容（500 字 provider/label）不折行（单测验证）。测试更新：live.test.mjs 改面板断言（panelBody 剥边框）+ 宽度/wrap 测试显式设 process.stdout.columns；**todo 完成后不消失**——实测父会话最后一次 todo/write 是两条 completed（模型从不写空列表），判定为 LLM 侧问题：面板侧兜底「全 completed 自动收起」（live.test.mjs 新增用例）+ **模型侧引导走 dsh 原生指令通道**（src/append-system.ts：**支持 pi 的 APPEND_SYSTEM.md 约定**（dsh 侧 ~/.dsh/APPEND_SYSTEM.md）——用户可编辑文件追加到本 TUI 所建 agent 的 system prompt，section text 用**函数 provider 每次装配时读文件**（改文件即热生效、无 watcher、空文件不贡献）；dsh-tui-pi:todo-lifecycle 段放同一文件（marker 注释幂等、tmp+rename 原子写、best-effort），并把旧通道 ~/.dsh/AGENTS.md 里的同段迁出（migrateAgentsMdTodoSection：精确块匹配剥离、清空则删文件；曾先试硬编码 ctx.systemPrompt.section 被用户否掉「改 system prompt 不太好」，又试 AGENTS.md 通道被用户指引用 pi 的 APPEND_SYSTEM.md）；新增 test/append-system.test.mjs 6 用例（建文件/追加/已标记不动/readAppendSystem/AGENTS.md 迁移/失败返回）；**最终形态**（用户拍板）：英文版内容放 `templates/APPEND_SYSTEM.md` 模板文件（**不写进代码**），安装/首次启动时 ensure 从模板播种 `~/.dsh/APPEND_SYSTEM.md`（模板 + 标记段；已有文件用户所有、只维护标记段），`~/.dsh/APPEND_SYSTEM.md` 已按英文版重装 | ✅ |
...
src/
  ...
  dsh-events.ts     本地声明 tool-workflow/agent-start|end、subagent/descriptor、llm/retry
                    ...
                    守卫 + AgentView 结构
  live-widgets.ts   LiveWidgets：两个带边框面板（tui.ts 的 dock VStack、编辑器正上方
                    widgets 槽，有内容才占行）——renderTodos（● Todos 树：header 计数 +
                    ├─/└─ + ☐/◐/☑，面板框 = top border + header 行 + body + bottom）/
                    renderAgents（● Agents 板，仅运行中子代理：spinner/provider+label/
                    ↻N≤M/token(+%)/耗时/⎿ activity；结算即消失、全空收起）/tickLive
                    （100ms 转 spinner/刷耗时）/setTheme/clear；todo/write 事件由
                    index.ts onEvent 转发（transcript 不再渲染）；行宽按 boxed inner
                    cap = panelBoxWidth-4 预算，provider+label 共享 nameBudget
