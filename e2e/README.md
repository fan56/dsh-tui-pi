...
| `69-btw` | **仅容器内**（复用 68 提交进 settings.yaml 的 `mock-llm` 路由与默认模型 `mock-chat`）：`/btw` 侧问全链路（`lib/mock-llm.mjs` 新增两个按请求体分相：`E2E_BTW_Q` 侧问快答——先于其他相位判定，因为侧问快照内嵌主线提示词；`E2E_BTW_MAIN` 主线慢流 ~42s 制造 running 窗口）。空闲三连：bare `/btw` 空 slot 回 usage 文案、idle 拒绝（`the main line is idle, so just ask directly`）、`--model` 缺问题内联报错。busy 全链：started 回显 + `⌘ btw — mock-llm/mock-chat` framed overlay、`Thinking…` → 流式答案；第二条 `/btw` 排队（回显 `btw queued (position 1)` + overlay 状态 `· 1 queued`）；落定徽标 `Not kept in the session · /btw reopens this answer`；排空任务在 overlay 未关时后台运行（`btw running in the background` 通知——hasCapturingSurface 跳过弹层）；bare `/btw` 重开 live overlay 流第二条答案；Esc 后两个答案标记均不在 transcript（「overlay 不落盘」的屏幕级证明——会话存储是 zstd 帧格式，文件级 grep 不可见）；主线不受扰动按期跑完 `E2E-BTW-MAIN-DONE`；Last-btw 复查（`Last btw exchange shown.` + 答案 + not-kept 徽标） |
| `71-cache-hit` | **仅容器内**（复用 68 的 `mock-llm` 路由；写 `cacheHitMode` 进容器 settings.yaml）：footer CH 段口径全链（`lib/mock-llm.mjs` 新增 `E2E_CH_TRIGGER` 两相位，usage 带 OpenAI `prompt_tokens_details.cached_tokens`——pi-ai 适配器映射为 cacheRead）：第一轮 900/1000 cached（该消息 CH 90%）、第二轮 200/1000（该消息 CH 20%），两轮全会话累计 55%——三个比率互异，footer 显示值即可鉴别模式。先重启 TUI 开净会话（累计分母=恰好两轮脚本），断言：无 usage 时无 CH 段 → 第一轮后 `CH90.0%` → 第二轮后默认 lastMessage 模式显示 `CH20.0%`（旧 90% 不残留、累计 55% 不泄漏）→ 外部编辑 settings.yaml 落 `cacheHitMode: session`（live watch 热应用，不重启）→ 同段翻成累计 `CH55.0%`、20% 消失 |
| `74-profiles` | **仅容器内**（写 `$DSH_HOME/model-profiles.json`、agents 目录与 cwd 钉文件，宿主机自跳过）：v2.2.0 model profiles 纯离线全链（模型路由取自 mock 供应商声明，选中模型不产生流量）。空店自愈：rm 掉 model-profiles.json 后 switcher 开出 work/personal/other 种子 roster + `.dsh-profile here: none`；Enter 应用 `work`——summary 回显 `Profile → work · model unchanged · agent values compose per-workspace at spawn · pinned this tree (.dsh-profile)`、`/app/.dsh-profile` 落盘、重开面板标题 `here: work` + `● work` 当前行；钉住时 `/agents` 字段窗宣告 profile 目标（`model/think edits apply to profile "work"`），模型编辑保存带 `(profile "work")` 作用域注记、override 落进 model-profiles.json（`"model":"mock-llm/mock-chat"`）而 agent 文件 frontmatter 字节不动；`p` 解钉（`unpinned /app` + 文件消失）；解钉后同一路径宣告 frontmatter 目标、保存无作用域注记、路由写进 agent 文件本身且 description 等未涉键原样保留 |
